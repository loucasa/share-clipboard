...
/**
 * Provides the UI for sharing a text with a {@link Device}.
 */
public class SendTextActivity extends Activity {

    /**
     * The request code for {@link SelectDeviceActivity}. This is used when the user doesn't select
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mContactId == Device.INVALID_ID) {
            selectContact();
        }
    }
        ...
        switch (requestCode) {
            case REQUEST_SELECT_CONTACT:
                if (resultCode == RESULT_OK) {
                    mContactId = data.getIntExtra(Device.ID, Device.INVALID_ID);
                }
    ...
    private boolean resolveIntent(Intent intent) {
        if (Intent.ACTION_SEND.equals(intent.getAction()) &&
            ...
            mContactId = intent.getIntExtra(Device.ID, Device.INVALID_ID);
            return true;
        }
        return false;
    }
    ...
    private void prepareUi() {
        if (mContactId != Device.INVALID_ID) {
            Device device = Device.byId(mContactId);
            DeviceViewBinder.bind(device, mTextContactName);
        }
        mTextMessageBody.setText(mBody);
    }

    /**
     * Delegates selection of a {@link Device} to {@link SelectDeviceActivity}.
     */
    private void selectContact() {
        Intent intent = new Intent(this, SelectDeviceActivity.class);
        intent.setAction(SelectDeviceActivity.ACTION_SELECT_DEVICE);
    ...
    private void send() {
        Toast.makeText(this,
                getString(R.string.message_sent, mBody, Device.byId(mContactId).getName()),
